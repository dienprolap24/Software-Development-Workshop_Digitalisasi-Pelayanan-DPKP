    ...
    try {
      ...
      if (tableExists) {
        ...
        if (emailColumnExists) {
          ...
          // If email column doesn't exist, we need to add it first without NOT NULL constraint
          // then populate it, then add the constraint
          console.log("🔧 Adding email column without constraints first...");
          
          try {
            // Check if email column already exists (double-check)
            const [recheckResults] = await sequelize.query(`
            ...
            if (recheckResults.length === 0) {
              ...
              `);
              console.log("✅ Email column added as nullable");
            } else {
              console.log("ℹ️ Email column already exists, skipping creation");
            }
            
            // Update all existing records with default email
            const [adminResults] = await sequelize.query(`
              SELECT COUNT(*) as count FROM admins;
            `);
            const adminCount = parseInt(adminResults[0].count);
            
            if (adminCount > 0) {
              console.log(`📧 Updating ${adminCount} existing admin records with default emails...`);
              await sequelize.query(`
                UPDATE admins 
                SET email = CONCAT('admin_', id, '@example.com')
                WHERE email IS NULL;
              `);
              console.log("✅ Updated existing admin records with default emails");
            }
            ...
            try {
              ...
              console.log("ℹ️ Email column already NOT NULL or error setting it:", notNullError.message);
            }
            ...
            try {
              ...
              if (constraintResults.length === 0) {
                ...
                `);
                console.log("✅ Email column set to UNIQUE");
              } else {
                console.log("ℹ️ Email unique constraint already exists");
              }
            } catch (uniqueError) {
              console.log("ℹ️ Email unique constraint already exists or error adding it:", uniqueError.message);
            }
